...
TOPIC_USERS_WILDCARD = f"{TOPIC_USERS}/+"
TOPIC_TOPICS_WILDCARD = f"{TOPIC_TOPICS}/+"
...
class UserApp(ctk.CTk):
    ...
    def handle_message(self, topic, payload):
        if topic == self.personal_topic:
            if payload: self.add_log(f"(Privado) de {payload}")
            ...
            topic_path = topic
            if not payload:
                if topic_path in self.topics: self.topics.remove(topic_path)
                if topic_path in self.active_subscriptions: self.active_subscriptions.discard(topic_path)
            elif payload == "ADD" and topic_path not in self.topics:
                self.topics.append(topic_path)
            self.update_topics_list_display()
            self.update_send_selectors()

        elif topic in self.active_subscriptions:
            self.add_log(f"({topic.replace(UNIQUE_PREFIX, '')}) | {payload}")
    ...
    def send_to_topic(self):
        topic_name_only = self.topic_combobox.get().strip()
        message = self.topic_msg_entry.get().strip()
        if not topic_name_only or "Selecione" in topic_name_only or "Nenhum" in topic_name_only or not message:
            self.add_log("ALERTA: Selecione um tópico e digite uma mensagem.")
            return
        full_topic_path = f"{TOPIC_TOPICS}/{topic_name_only}"
        full_message = f"{self.user_name}: {message}"
        self.mqtt_client.publish(full_topic_path, full_message)
        self.add_log(f"Você para ({topic_name_only}): {message}")
    ...
    def send_to_user(self):
        ...
        self.mqtt_client.publish(recipient_topic, payload, retain=False)
        self.add_log(f"Você para (Privado) {recipient}: {message}")
        self.user_msg_entry.delete(0, "end")
        
    def update_send_selectors(self):
        subscribed_topics = sorted([t.replace(f"{TOPIC_TOPICS}/", "") for t in self.active_subscriptions])
    ...
    def subscribe_to_topic(self, topic_path):
        self.mqtt_client.subscribe(topic_path)
        self.active_subscriptions.add(topic_path)
        self.add_log(f"Inscrito no tópico: {topic_path.replace(UNIQUE_PREFIX, '')}")
        self.update_topics_list_display()
        self.update_send_selectors()

    def unsubscribe_from_topic(self, topic_path):
        self.mqtt_client.unsubscribe(topic_path)
        self.active_subscriptions.discard(topic_path)
        self.add_log(f"Inscrição cancelada para: {topic_path.replace(UNIQUE_PREFIX, '')}")
        self.update_topics_list_display()
        self.update_send_selectors()
    ...
    def update_topics_list_display(self):
        ...
        for topic_path in sorted(self.topics):
            topic_name_only = topic_path.replace(f"{TOPIC_TOPICS}/", "")
            is_subscribed = topic_path in self.active_subscriptions
            
            btn_text = f"{topic_name_only} (Sair)" if is_subscribed else topic_name_only
            btn_fg_color = ("#4A4A4A", "#555555") if is_subscribed else ("#3B8ED0", "#1F6AA5")
            btn_command = lambda t=topic_path: self.unsubscribe_from_topic(t) if is_subscribed else self.subscribe_to_topic(t)
