...
class UserApp(ctk.CTk):
    ...
    def on_message(self, client, userdata, message):
        ...
        # Atualizações da lista de tópicos
    ...
    def setup_main_ui(self):
        ...
        # --- Frame da Esquerda (Listas e Ações) ---
        left_frame = ctk.CTkFrame(self)
        left_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        left_frame.grid_rowconfigure(1, weight=1)
        left_frame.grid_rowconfigure(3, weight=1)

        # Listas de Tópicos e Usuários
        ...
        self.user_msg_entry.grid(row=2, column=1, padx=10, pady=5, sticky="ew")
        send_user_button = ctk.CTkButton(right_frame, text="Enviar para Usuário (Offline)", command=self.send_to_user)
        send_user_button.grid(row=3, column=1, padx=10, pady=10, sticky="ew")
    ...
    def send_to_user(self):
        ...
        # O payload da mensagem privada é o remetente e a mensagem
        payload = f"{self.user_name}: {message}"
        recipient_topic = f"usuarios/{recipient}"
        self.mqtt_client.publish(recipient_topic, payload, retain=True) # Reter msg offline
    ...
    def update_topics_list_display(self):
        for widget in self.topics_list_frame.winfo_children():
            widget.destroy()
        
        for topic in sorted(self.topics):
            # Cria um botão para cada tópico para permitir a inscrição
            btn = ctk.CTkButton(self.topics_list_frame, text=topic, 
                                command=lambda t=topic: self.subscribe_to_topic(t))
